...
export const STAT_NAMES = {
  en: ['HP', 'Atk', 'Def', 'Sp.Atk', 'Sp.Def', 'Speed'],
  zh: ['体力', '攻击', '防御', '特攻', '特防', '速度'],
  ja: ['HP', 'こうげき', 'ぼうぎょ', 'とくこう', 'とくぼう', 'すばやさ'],
}

export type StatTuple = [number, number, number, number, number, number]

// Record<id, [hp, atk, def, spa, spd, spe]>
export const POKEMON_STATS: Record<number, StatTuple> = {
  // ── Gen 1 ────────────────────────────────────────────────────────────────
  1:  [45,  49,  49,  65,  65,  45],
  2:  [60,  62,  63,  80,  80,  60],
...
export function getStats(id: number): StatTuple {
  return POKEMON_STATS[id] ?? [50, 50, 50, 50, 50, 50]
}

export function totalStats(stats: StatTuple): number {
  return stats.reduce((a, b) => a + b, 0)
}

/** 0–255 → 0–100% for bar width */
export function statPercent(val: number): number {
  return Math.round((val / 255) * 100)
}

/** Color based on stat value */
export function statColor(val: number): string {
  if (val >= 130) return '#22c55e'   // green
  if (val >= 90)  return '#84cc16'   // lime
